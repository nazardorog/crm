...
public class WBS011_LoadExpensesAddDell {

    // Click Up:
    // CRM SEMI Truck
    // Load board
    // 11. Dispatch/Add expenses
    ...
    public void expensesAddDell () {
        ...
        // *** Додає Expense ***
        $(".dispatch-head-drivers i.fa-money").click();

        // Фрейм Add expenses
        $("#add_expenses .modal-title").shouldHave(text("Add expenses"));

        // Поле Name
        $("#loadexpenses-name").shouldBe(visible).setValue("Fuel is gasoline");
        ...
        // Поле Amount
        $("#loadexpenses-amount-disp").shouldBe(visible).setValue("1000");

        // Поле Location
        $("#loadexpenses-location").shouldBe(visible).setValue("New York");

        // Поле Note
        $("#loadexpenses-note").shouldBe(visible).setValue("additional filling");

        // Клік Submit фрейм Add expenses
        $("#update_load_expenses_send").click();

        //перевіряє доданий Expenses на фрейм Dispatch
        SelenideElement expensesDispatch = $$("td[colspan='2']")
                .findBy(text("Fuel is gasoline"));

        expensesDispatch.shouldBe(visible);
        expensesDispatch.shouldHave(text("Fuel is gasoline"));
        expensesDispatch.shouldHave(text("additional filling"));
        $$("tr[data-expense-key]").findBy(text("Fuel is gasoline")).shouldHave(text("New York"));
        $$("tr[data-expense-key]").findBy(text("Fuel is gasoline")).shouldHave(text("$10.00"));
