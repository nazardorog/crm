...
public class BigTruckTestCase7LoadBoard extends LoginUser2 {
    ...
    // CRM SEMI Truck
    // Load board
    // 7. Перевод обратно из Deliverd в En route
    ...
    @Test
    public void dеlivеredCargoToEnRout() throws InterruptedException {

        System.out.println("BigTruckTestCase7LoadBoard - Start");
...
//        $$(".dropdown-menu-right li").findBy(text("Mark as delivered")).shouldBe(enabled, Duration.ofSeconds(10)).click();
        ...
        //фрейм Bounce to en Route
        $("#view_item .modal-title")
                .shouldBe(visible, Duration.ofSeconds(5))
                .shouldBe(text("Bounce to en Route for Trip#" + loadNumber));

        //вводить текст в поле Reason
        $("#loadnotes-note").setValue("BigTruckTestCase7LoadBoard, Bounce to en Route for Trip#" + loadNumber);

        $("#return_en_route_send")
