...
public class BigTruckTestCase3ShippersReceivers {
    ...
    public void newShippersReceiversBigTruck() throws InterruptedException {
        ...
        System.out.println("BigTruckTestCase3ShippersReceivers - Start");
        ...
        //видаляє Shipper
        rowShippers.$("span.glyphicon-remove").click();

        //popap підтвердження видалення
        String popapText = switchTo().alert().getText();
        System.out.println("Alert says: " + popapText);

        assertThat(popapText).isEqualTo("Are you sure you want to delete this shipper receiver?");
        switchTo().alert().accept();

        //перевіряє видалення Trailer
        $("input[name='ShippersReceiversSearch[name]']").shouldBe(visible, Duration.ofSeconds(10)).setValue(atShippersName).pressEnter();
        $(".empty").shouldBe(text("No results found."));

        System.out.println("BigTruckTestCase3ShippersReceivers - Test Pass");
    }
