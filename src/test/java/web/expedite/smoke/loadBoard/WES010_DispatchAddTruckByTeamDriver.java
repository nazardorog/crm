...
@Epic("Expedite")
@Feature("Smoke")
public class WES010_DispatchAddTruckByTeamDriver {

    // Click Up:
    // CRM EXPEDITE - Smoke - LoadBoard
    // 9. Создание New Load / dispatch load \ добавление трака на груз через поле тим драйвер

    @Test(description = "тест в description")
    @Story("Load board")
    @Description("дескріпш")
    @Severity(SeverityLevel.CRITICAL)
    public void addTruckByTeamDriver() {

        // Встановлюємо кастомну назву для тесту
        Allure.getLifecycle().updateTestCase(testResult -> {
            testResult.setName("Создание New Load / dispatch load \\ добавление трака на груз через поле тим драйвер");
        });
