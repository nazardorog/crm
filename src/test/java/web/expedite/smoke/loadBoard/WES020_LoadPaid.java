...
public class WES020_LoadPaid {
    ...
    public void loadPaid () {
        ...
        // Выход из аккаунта
        executeJavaScript("document.querySelector('.toast.toast-success')?.remove();");
        ...
        $("#invoice-loads-grid-filters input[name='LoadsSearch[our_pro_number]']").shouldBe(visible, EXPECT_GLOBAL).setValue(pro_number).pressEnter();

        // Чекбокс Load paid
        $("input.container-checkbox-green[value='" + pro_number + "']").shouldBe(visible, EXPECT_GLOBAL);
        executeJavaScript("arguments[0].checked = true;", $("input.container-checkbox-green[value='" + pro_number + "']"));
        ...
        $("#paid-loads-grid td.our_pro_number").shouldBe(visible, EXPECT_5).shouldHave(text(pro_number));
    }
