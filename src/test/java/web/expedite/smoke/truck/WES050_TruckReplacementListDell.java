...
public class WES050_TruckReplacementListDell {
    ...
    public void replacementListDell() {
        ...
        // [Main Owners] Table. Check added Driver
        $("input[name='OwnersSearch[name]']").shouldBe(visible).setValue(newTruck.atOwner).pressEnter();
        SelenideElement rowTable = $$("table.table-striped tbody tr").get(0).shouldHave(text(newTruck.atOwner), EXPECT_GLOBAL);
        rowTable.shouldHave(text(newTruck.atOwner));
        rowTable.$(".units").shouldHave(text(newTruck.atTruckNumber));
        ...
        // [Replacement List] Table. View truck
        ...
        // [Replacement List] Table. Delete truck
        rowTableReplacementList.$(".delete_truck").click();
        $("#delete_truck").shouldBe(visible, EXPECT_GLOBAL);
        $(byText("Delete")).click();
        ...
        // [Main Owners] Table. Check delete Driver
        $("input[name='OwnersSearch[name]']").shouldBe(visible).setValue(newTruck.atOwner).pressEnter();
        SelenideElement rowTableAfterDell = $$("table.table-striped tbody tr").get(0).shouldHave(text(newTruck.atOwner), EXPECT_GLOBAL);
        rowTableAfterDell.shouldHave(text(newTruck.atOwner));
        rowTableAfterDell.$(".units").shouldNotHave(text(newTruck.atTruckNumber));
    }
