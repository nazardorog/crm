...
public class WES048_TruckReplacementListView {
    ...
    public void replacementListView() {
        ...
        rowTableReplacementList.$(".glyphicon-eye-open").click();

        // [View truck] Check data Truck in Replacement List
        $("#view_truck").shouldBe(visible, EXPECT_GLOBAL);
        $$("table#w0 tr").findBy(text("Vin Number"))            .$$("td").first().shouldHave(text(newTruck.atVinNumber));
        $$("table#w0 tr").findBy(text("Plate Number"))          .$$("td").first().shouldHave(text(newTruck.atPlateNumber));
        $$("table#w0 tr").findBy(text("Make"))                  .$$("td").first().shouldHave(text(newTruck.atMake));
        $$("table#w0 tr").findBy(text("Model"))                 .$$("td").first().shouldHave(text(newTruck.atModel));
        $$("table#w0 tr").findBy(text("Type Truck"))            .$$("td").first().shouldHave(text(newTruck.atTypeTruck));
        $$("table#w0 tr").findBy(text("Year"))                  .$$("td").first().shouldHave(text(newTruck.atYear));
